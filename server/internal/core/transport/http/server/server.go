...
func NewHTTPServer(config ServerConfig, log *core_logger.Logger, middleware ...core_http_middleware.Middleware) *HTTPServer {
	return &HTTPServer{
...
func (h *HTTPServer) Run(ctx context.Context) error {
	...
	case <-ctx.Done():
		...
		h.log.Warn("HTTP server shutdown sucessful")
	case err := <-ch:
		if err != nil {
